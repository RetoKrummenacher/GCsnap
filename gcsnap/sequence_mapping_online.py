...
class SequenceMappingOnline:
    ...
    def run(self):
        ...
        # check and error logging is done here. 
    ...
    def get_codes(self, id_type: str = None) -> list:
        ...
        return self.mapping_df[id_type].tolist()
    ...
    def create_mapping_df(self, mapping_dict: dict) -> pd.DataFrame:        
        ...
        for id_type in list(set(id_types).difference(set([from_type,to_type]))):
            mappings[id_type] += [np.nan] * len(mapping_dict['from_id'])
                
        return pd.DataFrame.from_dict(mappings)

    def make_unique(self, mapping_df: pd.DataFrame, from_type: str, to_type: str) -> pd.DataFrame:
        ...
        df['Length'] = df[to_type].apply(len) 
        # sort, default ascending
        df = df.sort_values(by='Length')             
        
        # is duplicated detects Nan as duplicated, hence most rows are dropped for GeneID
        mask = df[from_type].notna()

        # Checking for duplicates only on non-NaN values while preserving the original index
        is_duplicated = df.loc[mask, from_type].duplicated(keep=False).reindex(df.index, fill_value=False)
        ...
        first_non_unique_df = df[is_duplicated].groupby(from_type, sort=False).head(1)
    ...
    def log_failed(self) -> None:
        ...
        # Combine all non-NaN entries from the specified columns into one list
        non_nan_entries = [df[col].dropna().tolist() for col in
                            list(set(self.supported).difference(set([self.to_type])))]
        # Flatten the list of lists
        non_nan_entries = [entry for sublist in non_nan_entries for entry in sublist]
        # Create a set of all non-NaN entries
        non_nan_entries = set(non_nan_entries)

        for id in list(set(self.target_list).difference(non_nan_entries)):
            logger.warning(f'Failed to map id {id} to {self.to_type}')    
